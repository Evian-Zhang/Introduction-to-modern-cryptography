...
bitset<32> Rcon[10] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
...
void SubByte(bitset<8> state[4][4], CryptoMode cryptoMode)
{
    ...
    for (int row = 0; row < 4; row++)
        for (int column = 0; column < 4; column++)
        {
            ...
            int S_box_row = previousValue[7] * 8 + previousValue[6] * 4 + previousValue[5] * 2 + previousValue[4];
            int S_box_column = previousValue[3] * 8 + previousValue[2] * 4 + previousValue[1] * 2 + previousValue[0];
            *(*(state + row) + column) = crypto_S_box[S_box_row][S_box_column];
        }
}
...
void ShiftRows(bitset<8> state[4][4], CryptoMode cryptoMode)
{
    for (int row = 0; row < 4; row++)
    {
        ...
        switch (cryptoMode)
        {
            case Enc:
                for (int column = 0; column < 4; column++)
                    state[row][column] = previousRow[(column - shiftBytes[row] + 4) % 4];
                break;
                
            case Dec:
                for (int column = 0; column < 4; column++)
                    state[row][column] = previousRow[(column + shiftBytes[row]) % 4];
                break;
...
void MixColumns(bitset<8> state[4][4], CryptoMode cryptoMode)
{
    ...
    for (int column = 0; column < 4; column++)
    {
        ...
        for (int row = 0; row < 4; row++)
        {
            state[row][column] = 0;
            for (int i = 0; i < 4; i++)
                state[row][column] ^= *(*(*(*crypto_M + row) + i) + previousColumn[row].to_ulong());
        }
    }
}
    ...
    for (int row = 0; row < 4; row++)
        for (int column = 0; column < 4; column++)
            for (int bit = 0; bit < 8; bit++)
                state[row][column][bit] = ki[row][column * 4 + bit] ^ state[row][column][bit];
}
...
bitset<32> SubWord(bitset<32> input)
{
    ...
    {
        int row = input[8 * byte + 7] * 8 + input[8 * byte + 6] * 4 + input[8 * byte + 5] * 2 + input[8 * byte + 4];
        int column = input[8 * byte + 3] * 8 + input[8 * byte + 2] * 4 + input[8 * byte + 1] * 2 + input[8 * byte];
...
void bitToState(bitset<128> input, bitset<8> state[4][4])
{
    for (int column = 0; column < 4; column++)
        for (int row = 0; row < 4; row++)
            for (int bit = 0; bit < 8; bit++)
                state[row][column][bit] = input[32 * column + 8 * row + bit];
}
...
void stateToBit(bitset<8> state[4][4], bitset<128> &output)
{
    for (int column = 0; column < 4; column++)
        for (int row = 0; row < 4; row++)
            for (int bit = 0; bit < 8; bit++)
                output[32 * column + 8 * row + bit] = state[row][column][bit];
}
...
{
    string plainTextStr = "00010001001001101010101011001111000100110101011110011011110111100000001001001101000101011001111000100110101111110011011110111100";
    bitset<128> plainText(plainTextStr);
    cout << plainText.to_string() << endl;
    bitset<128> key(plainTextStr);
    cout << AES_128_DEC(AES_128_ENC(plainText, key), key).to_string() << endl;
